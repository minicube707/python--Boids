...
class Boids():
    ...
    tab_norme = np.array([[]], dtype=np.int16)

    def __init__(self,  colour=tuple(), len=int(), speed_max=int(),  perception=int(), max_force=float()) -> None:
    ...
    def calcul_distance(self, index=int(), list_boids=np.array([])):

        """"""""""""""""""""""""""""""""""""""
        #Récupératoin des distance
        """"""""""""""""""""""""""""""""""""""
        #Initialisation
        list_norme = np.array([],dtype=np.int16) 
        
        #Si c'est le premier boid initialisation du tableau des distances communes
        if index == 0:
            Boids.tab_norme = np.array([], dtype=np.int16)
        ...
        for other_boids in list_boids[index+1:]:
            ...
            norme = np.int16(np.linalg.norm(other_boids.position - self.position))
    ...
    def comportement(self, index=int(), list_boid=np.array([]), filter=np.array([])):
